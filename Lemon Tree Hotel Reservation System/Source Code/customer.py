...
class CustomerFactory:
    ...
    def create_customer(customer_data):
        required_fields = ["FullName", "DateofBirth", "PhoneNumber", "Email", "CustomerType"]
        missing_fields = [field for field in required_fields if not customer_data.get(field)]

        # Check for missing required fields
        if missing_fields:
            messagebox.showinfo("Missing Fields", "Please fill in the required fields",
                                parent=master)
...
class Customer:
    def __init__(self, master):
        ...
        self.master.geometry("1222x570+0+520")

        # Change the background color of self.master
        self.master.configure(bg="#d84c42")
        ...
        # TITLE BANNER
        title_banner_image = Image.open("/Users/pop/Downloads/topbanner.png")
        title_banner_image = title_banner_image.resize((1222, 50), Image.Resampling.LANCZOS)
        self.phototitle_banner_image = ImageTk.PhotoImage(title_banner_image)

        label_img = Label(master, image = self.phototitle_banner_image, bd=0, relief=RIDGE)
        label_img.place(x=0, y=0, width=1222, height=50)

        ######### Label Frame Left Side #########
        # Create left-side frame for customer details input
        labelframe_ls = LabelFrame(master, bd = 0, relief = RIDGE, text = "",
                                   font = ("arial", 12, "bold"), bg="#d84c42",padx = 2)
        ...
        label_customer_id = Label(labelframe_ls, text = "Customer ID",
                                  font=("arial", 12, "bold"), bg="#d84c42",padx=2, pady=6)
        ...
        label_customer_title = Label(labelframe_ls, text = "Title",
                                     font=("arial", 12, "bold"), padx=2, bg="#d84c42", pady=6)
        label_customer_title.grid(row=1, column=0, sticky=W)

        # Customer TITLE COMBOBOX
        # Select title field
        combo_title = ttk.Combobox(labelframe_ls, textvariable=self.var_customer_title, font=("arial", 13, "bold"), width=27, state="readonly")
        ...
        dob_input = DateEntry(labelframe_ls, textvariable=self.var_customer_dob, width=27, font=("arial", 13, "bold"),
        ...
        combo_gender = ttk.Combobox(labelframe_ls, textvariable=self.var_customer_gender, font=("arial", 13, "bold"), width=27, state="readonly")
        ...
        combo_country = ttk.Combobox(labelframe_ls, textvariable=self.var_customer_country, font=("arial", 13, "bold"), width=27, state="readonly")
        ...
        combo_reward = ttk.Combobox(labelframe_ls, textvariable=self.var_customer_type, font=("arial", 13, "bold"), width=27, state="readonly")
        ...
        ######### BUTTON FRAME #########
        button_frame = Frame(labelframe_ls, bd=0, bg="#d84c42",relief=RIDGE)
        button_frame.place(x=4, y=340, width=352, height=139)


        # ADD CUSTOMER
        add_customer_button = Button(button_frame, text="Submit Details", command=self.insert_data, font=("arial", 16, "bold"), bg="#d84c42",
                                     fg="#d84c42", width=34)
        ...
        mysql_table_frame = LabelFrame(self.master, bd=1, relief=RIDGE, text="Customer Search System", font=("arial", 12, "bold"), bg="#d84c42",padx=2)
        ...
        label_search.grid(row=0, column=0, sticky=W,pady=2, padx=2)
        ...
        combobox_search.grid(row=0, column=1, pady=2,padx=2)


        # Search LABEL
        label_search = Label(mysql_table_frame, text = "Keyword", bg="yellow", fg="black")
        label_search.grid(row=1, column=0, sticky=W, pady=2,padx=2)


        # Search INPUT
        self.search_input=StringVar()
        search_input = ttk.Entry(mysql_table_frame, width=26, textvariable=self.search_input, font=("arial", 13, "bold"))
        search_input.grid(row=1, column=1, padx=2, pady=2,  sticky=W)


        # Search BUTTON
        search_button = Button(mysql_table_frame, text="Search Details", command=self.search_data, font=("arial", 11, "bold"), bg="#d84c42",
                               fg="#d84c42", width=26)
        search_button.grid(row=2, column=1, padx=2, pady=4)

        # ShowAll BUTTON
        show_all_button = Button(mysql_table_frame, text="Show All Details", command=self.fetch_data, font=("arial", 11, "bold"), bg="#d84c42",
                                 fg="#d84c42", width=26)
        show_all_button.grid(row=3, column=1, padx=2, pady=4)

        # Right side IMAGE
        img3 = Image.open("/Users/pop/Downloads/righsidebanner2.png")
        img3 = img3.resize((514, 118), Image.Resampling.LANCZOS)
        self.photoimg3 = ImageTk.PhotoImage(img3)

        label_img = Label(self.master, image = self.photoimg3, bd=0, relief=RIDGE)
        label_img.place(x=680, y=68, width=514, height=118)
        ...
        display_data_table.place(x=3, y=127, width=810, height=350)
    ...
    def insert_data(self):
        ...
        if (self.var_customer_phone_number.get()==""
                or self.var_customer_dob.get()==""
                or self.var_customer_name.get()==""
                or self.var_customer_email.get()==""
                or self.var_customer_type.get()==""):
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success", "The customer details have been saved!", parent=self.master)
